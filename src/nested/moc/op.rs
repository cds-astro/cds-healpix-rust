        ...
        Some(curr) => {
          if !self.to_be_possibly_packed.is_empty() {
            ...
            if curr.depth == prev.depth && curr.hash == prev.hash + H::one() {
              ...
              && curr.hash == (prev.hash + h1) << (((curr.depth - prev.depth) << 1) as usize)
            {
  ...
  T2: Iterator<Item = H>,
{
  pub fn new(
    ...
    mut left_it: T1,
    mut right_it: T2,
  ) -> MinusOnSingleDepthCellsIter<H, T1, T2> {
  ...
  fn unpacked_next(&mut self) -> Option<HpxCell<H>> {
    ...
    match (&self.left, &self.right) {
      (Some(l), Some(r)) => match l.hash.cmp(&r.hash) {
  ...
  T: MOCIterator<H>,
{
  ...
  fn go_up_if_needed(&mut self) {
    let dd = self
      .curr_d
      .min(self.curr_h.trailing_zeros() as u8 / 2)
      .min(self.curr_d);
    self.curr_d -= dd;
    self.curr_h >>= dd << 1;
  }
  ...
  fn next(&mut self) -> Option<Self::Item> {
    match &self.curr {
      Some(c) => {
        ...
        while self.curr_d < c.depth
          && self.curr_h == (c.hash >> ((c.depth - self.curr_d) << 1) as usize)
        {
          self.curr_d += 1;
          self.curr_h <<= 2;
        }
  ...
  fn next(&mut self) -> Option<Self::Item> {
    while let (Some(l), Some(r)) = (&self.left, &self.right) {
      match l.depth.cmp(&r.depth) {
        ...
        Ordering::Equal => match l.hash.cmp(&r.hash) {
          Ordering::Less => self.left = self.left_it.next(),
          Ordering::Greater => self.right = self.right_it.next(),
          Ordering::Equal => {
  ...
  T2: MOCIterator<H>,
{
  ...
  fn unpacked_next(&mut self) -> Option<HpxCell<H>> {
    ...
    match (&self.left, &self.right) {
      (Some(l), Some(r)) => match l.depth.cmp(&r.depth) {
        Ordering::Less => {
          let hr_at_dl = r.hash.unsigned_shr(((r.depth - l.depth) << 1) as u32);
          match l.hash.cmp(&hr_at_dl) {
  ...
  T2: Iterator<Item = HpxCell<H>>,
{
  ...
  while let Some(c) = &curr {
    if c.depth <= low_res_cell.depth
      || low_res_cell.hash
        != c
          .hash
          .unsigned_shr(((c.depth - low_res_cell.depth) << 1) as u32)
    {
      break;
    } else {
      curr = it.next();
